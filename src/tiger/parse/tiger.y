...
// fundec
fundec:  FUNCTION  fundec_one  {$$ = new absyn::FunDecList($2);}
  |  FUNCTION  fundec_one  fundec  {$$ = $3->Prepend($2);}
  ;

fundec_one:  ID  LPAREN  tyfields  RPAREN  COLON  ID  EQ  exp  {$$ = new absyn::FunDec(scanner_.GetTokPos(), $1, $3, $6, $8);}
  |  ID  LPAREN  tyfields  RPAREN  EQ  exp  {$$ = new absyn::FunDec(scanner_.GetTokPos(), $1, $3, NULL, $6);}
  ;

// tydec
tydec:  TYPE  tydec_one  {$$ = new absyn::NameAndTyList($2);}
  |  TYPE  tydec_one  tydec  {$$ = $3->Prepend($2);}
  ;

tydec_one:  ID  EQ  ty  {$$ = new absyn::NameAndTy($1, $3);}
  ;

ty:  LBRACE  tyfields  RBRACE  {$$ = new absyn::RecordTy(scanner_.GetTokPos(), $2);}
...
expseq:  {$$ = new absyn::VoidExp(scanner_.GetTokPos());}
  |  exp  {$$ = $1;}
  |  exp  SEMICOLON  sequencing  {$$ = new absyn::SeqExp(scanner_.GetTokPos(), $3->Prepend($1));}
  ;
...
exp:
  ...
  // let
  |  LET  decs  IN  sequencing  END  {$$ = new absyn::LetExp(scanner_.GetTokPos(), $2, new absyn::SeqExp(scanner_.GetTokPos(), $4));}
  // array
